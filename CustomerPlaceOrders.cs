...
{
    public partial class CustomerPlaceOrders : UserControl
    {
        ...
        private void CustomerOrder_payOrder_Click(object sender, EventArgs e)
        {
            //IDGenerator();
            ...
            //                    cmd.Parameters.AddWithValue("@change", cashierOrder_amount.Text);
            ...
            {
                DialogResult paymentChoice = MessageBox.Show("Do you want to pay using bKash?\nClick 'Yes' for bKash or 'No' for Visa.",
                    "Select Payment Method", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);
                ...
                string paymentMethod = (paymentChoice == DialogResult.Yes) ? "bKash" : "Visa";
                ...
                if (MessageBox.Show("Are you sure to pay your orders?", "Confirmation message", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    if (checkConnection())
                    {
                        try
                        {
                            ...
                            decimal totalPrice = decimal.Parse(cashierOrder_totalPrice.Text);
                            decimal amount = decimal.Parse(cashierOrder_amount.Text);
                            decimal change = (paymentMethod == "bKash" || paymentMethod == "Visa") ? 0 : amount - totalPrice;

                            string insertData = "INSERT INTO customers (customer_id, total_price, amount, change, order_Date, PaymentMethod) " +
                                                "VALUES (@cID, @totalPrice, @amount, @change, @date, @paymentMethod)";
                            using (SqlCommand cmd = new SqlCommand(insertData, connect))
                            {
                                cmd.Parameters.AddWithValue("@cID", idGen);
                            ...
                            clearFeilds();
                            MessageBox.Show("Paid successfully via " + paymentMethod, "Information message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                        catch (Exception ex)
                        {
        ...
        private void CustomerOrder_Recipt_Click(object sender, EventArgs e)
        {
            if (cashierOrder_amount.Text == "" || dataGridView3.Rows.Count < 0)
            {
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            e.Graphics.DrawString("Total price : \tBDT" + totalPrice + "\nAmount :\tBDT" + cashierOrder_amount.Text.Trim()
                + "\n\t\t-----------------\nChange : \tBDT", labelFont, Brushes.Black, labelX, y);
